...
class Fred(object):
    ...
    def search_by_country(self, country_text, limit=0, order_by=None, sort_order=None, filter=None):
        """
        ...
        country_category_table = {'Taiwan' : 32438, 'Japan' : 32281,
                                  'South Korea' : 32286, 'Hong Kong' : 32727,
                                  'India' : 32333, 'China' : 32329, 'Singapore': 32287,
                                  'United States' : 32267, 'Canada' : 32268,
                                  'United Kingdom' : 32280, 'France' : 32272,
                                  'Germany' : 32273, 'Italy': 32274, 'Israel' : 32335,
                                  'Sweden' : 32278, 'Switzerland' : 32279, 'Brazil' : 32327,
                                  'Denmark' : 32271, 'Egypt' : 32697, 'South Africa' : 32345,
                                  'Australia' : 32269, 'New Zealand' : 32338, 'Austria' : 32282,
                                  'Russian Federation' : 32341, 'Romania' : 32793,
                                  'Hungary' : 32331, 'Thailand' : 32823}

        try:
            country_id = country_category_table[country_text]
        except KeyError:
            print("Country not found in dictionary.")
                                  
       
        url = "%s/category/series?category_id=%d&" % (self.root_url, country_id)
        info = self.__get_search_results(url, limit, order_by, sort_order, filter)
        if info is None:
            raise ValueError('No series exists for such country: ' + str(country_text))
        return info

    def search_by_state(self, state_text, limit=0, order_by=None, sort_order=None, filter=None):
        """
        Search for series that belongs to a category id. Returns information about matching series in a DataFrame.
        ...
        state_category_table = {'AL' : 27282, 'NE' : 32281,
                                'AK' : 27283, 'NV' : 27310,
                                'AZ' : 27284, 'NH' : 27311,
                                'AR' : 149,   'NJ' : 27312,
                                'CA' : 27286, 'NM' : 27313,
                                ...
                                'MT' : 27308}
        try:
            state_id = state_category_table[state_text]
        except KeyError:
            print("State not found in dictionary.")
                                  
       
        url = "%s/category/series?category_id=%d&" % (self.root_url, state_id)
        info = self.__get_search_results(url, limit, order_by, sort_order, filter)
        if info is None:
            raise ValueError('No series exists for such state: ' + str(state_text))
        return info
